...
Everything known to be outstanding in this package after M5, with the evidence for each. Written
as issues rather than prose so they can be filed as-is when this repo gets a remote. Defects found
...
The fix (`src/harness/compat.ts`, `makeSharedProvider`) is to `share()` one subscription between the
...
While wiring it, `VERIFIED_CORE` in `src/harness/compat.ts` was found exported and read by
