...
test("star of 4: the hub is adjacent to everyone, and that is not a failure", async () => {
  ...
  // non-neighbour by construction, so arm 1 can say nothing about it — the old
  ...
  // non-neighbours, and the guarantee is established for them.
...
test("a disconnected graph: an isolated participant is a result, not a hang", async () => {
  ...
  // a neighbour, so a wait on `neighbors.length > 0` would sit until the timeout
  ...
  // generator's REALISED graph differs between runs of this same test. Expected
...
test("a complete graph is refused rather than passed", async () => {
  ...
  assert.equal(result.candidatePairs, 0, "there is no non-neighbour to leak to");
