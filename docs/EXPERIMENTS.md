...
The argument gets stronger, not weaker, at the wider radii the package now supports. Each step out makes the coordination task easier in the same way and by more, and `graph: { projectFar }` compounds it by letting distant participants' colours reach a subject as well as distant ties — which for a colouring game is most of the solution. At `radius: "whole"` the paper's dependent variable is not measurable at all: a subject who can see the entire graph can solve it immediately, and the session ends in seconds while the interface still looks correct. None of that is a reason not to have the settings; it is a reason the default is 1 and every one of them is recorded. `test/e2e/shirado2017.test.ts` asserts the absence of the projected field; the radius is a configuration rather than a projection, so it is recorded instead — `readRadius(game)` reads back what any given run actually used ([DATA-AND-ANALYSIS.md](DATA-AND-ANALYSIS.md#4-reproducing-a-finished-run)).
