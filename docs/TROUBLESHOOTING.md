...
`classicKinds`. `docs/GETTING-STARTED.md` Section 3 has the diff, and the warning prints it too.
...
If it is already registered and this warning still appears, the subscription was merely slow, and
the package will retract the warning itself the moment a channel arrives:
...
means your setup beat the measurement, which is worth reporting along with the participant count.
...
`ISSUES.md` O11 · `docs/GETTING-STARTED.md` Section 4
...
The same trap arrives by a different route inside `onPrivateState`: the hook is synchronous, a
...
This is not a bug: `undefined` means no view has been published to this participant yet, whereas
`[]` means a genuinely isolated node. The hook refuses to conflate them, because returning `[]`
while loading would render a participant as isolated, look entirely normal, and quietly corrupt
the data.
...
Fix: add the key to `watch`. (`read` would work identically here, but `watch` is the one that
says "the projection depends on this".)
...
Fix: this must be in place before the run. Turn on the run log and write to it as you go.
...
`docs/DATA-AND-ANALYSIS.md` Section 6 · `docs/GETTING-STARTED.md` Section 9
...
A policy that throws is caught, logged, and swallowed, because one failing agent must not
leave the study a player short, so a bot that stopped acting after one tick is in the log, not in
an exception.
...
`.js` file as CommonJS; the shared rules module is ESM. The stack names your file and not
...
Two different causes apply, in this order.

First cause: orphaned harness servers. The Empirica CLI execs a versioned binary as its own
child, so killing the CLI leaves the real server running and holding its port. Accumulated
orphans starve player assignment.

```sh
pkill -f "empirica-networks-.*tajriba.toml"
```

Second cause: whole-run weight. `npm test -- e2e` is the reliable signal. A full `npm test`
