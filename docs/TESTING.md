...
unbundled (`docs/PLATFORM-NOTES.md`, Section 3a). That is why they can run directly under `tsx`
and why they are separated from e2e in continuous integration (CI).
...
`undefined` and `[]` stay distinct, and, which is the reason this tier exists, that the `dones`
...
mode (`docs/PLATFORM-NOTES.md`, Section 8); that is a limit of our tests, not a consumer-facing one.
