...
import { parseNdjson, positionRows, structureRows, toCSV } from "empirica-networks/export";
...
for (const [gameID, gameRecords] of byGame) {
  ...
  const beyond = ties.filter((t) => t.a_index !== 0 && t.b_index !== 0).length;
  console.log(
    `${gameID}: ${ties.length} tie row(s) (${beyond} between neighbors), ` +
      `${nodes.length} position row(s) -> ${dir}`
  );
}

if (!wroteAny) {
  console.log(
    "Nothing written. Run the study with NBHD_RADIUS=1.5 to deliver structure worth recovering."
  );
}
