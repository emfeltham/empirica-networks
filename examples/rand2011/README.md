...
paper. No data has been collected with this code, nothing has been compared to the authors'
results, and nothing here supports or challenges their findings. It is not "ported from
Breadboard" either — no code from the original exists to port. If you run it and write about it,
...
## Where each value lives, and why that is the whole design
...
measured *not* to be delivered to participants (`docs/PLATFORM-NOTES.md` §4c).
...
- Stochastic session length: 80% chance of another round, drawn from a seeded rng so the realised
...
**The CSVs above are written when a game ENDS.** A study that is killed, crashes, or is stopped
mid-session never reaches that — and after `ISSUES.md` U2 a crash mid-study is the *normal* shape
of "something went wrong", because a restarted server cannot put participants back in their game
anyway. The session is over whether you like it or not, and the rounds that did complete are real
data.

So the experiment also appends **`data/run.ndjson` as it runs** — one record per scored round, plus
the condition and the edge events. Written through the package's `net.log()` (`log: { file }` in
`callbacks.js`), which stamps each record with its `gameID` and the moment it was written, so ONE
file holds a whole study and a batch of concurrent games interleaves safely. Rebuild the CSVs from
it:
...
Not approximately the same tables: `test/unit/rand2011.test.ts` asserts the recovered
`rounds.csv` is **byte-identical** to one written at a clean finish, because recovered data that
quietly differs from normal data is worse than no recovery. Checked end to end as well as in unit
tests — for a session of this suite's that happened to end naturally, `rounds.csv`, `edges.csv` and
...
- `test/e2e/rand2011.test.ts` — this experiment's `callbacks.js`, **imported unmodified**, against
  ...
  and reach only their decider, and an answer really does change the graph and get logged.
- `npm run example:build rand2011` (from the repo root) compiles the client. CI runs it for every
...
## Two traps this file exists to have already hit
...
- **`onStageEnded` can only be registered once.** Empirica's lifecycle helpers are wrapped in a
  ...
  file was written with one handler per stage; the second was dead. See
